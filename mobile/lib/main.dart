...
contentTextStyle: TextStyle(color: _softWhite),
),
...
try {
  ...
  setState(() {
    servers = realServers.isEmpty
        ? demoServers
        : realServers.map(DashboardServer.fromEmpServer).toList();
    selectedServer = 0;
    error = null;
  });
} catch (exception) {
...
widget.api.currentUser == null ||
identical(servers, demoServers);
...
DashboardTab.activity => _ActivityPage(
mode: mode,
server: current,
api: widget.api,
),
DashboardTab.security => _SecurityPage(
mode: mode,
server: current,
api: widget.api,
),
DashboardTab.about => _AboutPage(
mode: mode,
api: widget.api,
user: widget.api.currentUser,
error: error,
...
throw Exception('Enter any username/email and password to start demo mode.');
  ...
  if (mounted) {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Demo login active. Backend/plugin required for real controls.'),
      ),
    );
  }
  return;
...
widget.api.currentUser = EmpUser(
'demo-owner',
displayName.isEmpty ? 'demo' : displayName,
'Demo Owner',
'OWNER',
);
...
final message = demo
? 'Demo mode: action preview only. Connect backend/plugin to use remote server controls.'
: '$action queued through EMP Control backend.';
...
ScaffoldMessenger.of(context).showSnackBar(
const SnackBar(
content: Text('Files are disabled in the MVP. No server files are exposed.'),
),
);
return;
}
...
if (title == 'Console' &&
    widget.api.currentUser?.owner != true &&
    mode == ConnectionMode.connected) {
  ScaffoldMessenger.of(context).showSnackBar(
    const SnackBar(content: Text('Console is OWNER-only when auth is active.')),
  );
  return;
}
...
colors: [_purple, _neon],
),
boxShadow: [
BoxShadow(
color: _purple.withOpacity(.62),
blurRadius: 34,
spreadRadius: 2,
),
],
...
children: [
_StatusPill(
text: _Header.modeLabel(widget.mode),
color: widget.mode == ConnectionMode.connected
? Colors.greenAccent
: _neon,
),
_StatusPill(
...
icon: const Icon(Icons.refresh, color: _softWhite),
),
],
),
...
const Text(
'Demo mode works without backend. Real server control requires backend + plugin.',
...
child: Text(
busy ? 'Connecting...' : 'Enter Control Panel',
...
borderRadius: BorderRadius.circular(18),
borderSide: BorderSide(color: _purple.withOpacity(.25)),
),
...
_StatusPill(
text: modeLabel(mode),
color: mode == ConnectionMode.connected
? Colors.greenAccent
: _neon,
),
_StatusPill(
...
static String modeLabel(ConnectionMode mode) => switch (mode) {
    ...
    children: [
      ...
      SizedBox(
        ...
        child: ListView.separated(
          ...
          itemBuilder: (context, index) => _ServerChip(
        ...
        children: [
          ...
          Expanded(
            child: _ControlButton(
              ...
              onTap: () => onControl('Restart Server'),
            ),
          ),
        ],
      ),
          ...
          _FeatureSpec('Stats', Icons.query_stats_rounded, 'Live health'),
            ...
            .map(
              (feature) => _FeatureCard(
                spec: feature,
                onTap: () => onOpenFeature(feature.title),
              ),
            )
            .toList(),
      ),
...
colors: selected
? [_panel2, const Color(0xFF32135C)]
: [_panel, const Color(0xFF14101E)],
),
border: Border.all(
color: selected ? _neon : Colors.white10,
width: selected ? 1.4 : 1,
),
boxShadow: selected
? [
BoxShadow(
color: _purple.withOpacity(.52),
blurRadius: 26,
spreadRadius: 1,
),
]
...
children: [
_StatusPill(
text: server.online ? 'SERVER ONLINE' : 'SERVER OFFLINE',
color: server.online ? Colors.greenAccent : Colors.redAccent,
),
const Spacer(),
...
Text(
'${server.mode} • ${server.software} • ${server.version}',
...
_MetricTile(
label: 'TPS',
value: server.tps.toStringAsFixed(1),
...
gradient: LinearGradient(
colors: danger
? [const Color(0xFF3A1118), const Color(0xFF251018)]
: [_purple.withOpacity(.95), _panel2],
),
border: Border.all(
color: danger
...
borderRadius: BorderRadius.circular(22),
border: Border.all(
color: spec.title == 'Files'
? Colors.white10
: _purple.withOpacity(.32),
),
),
child: Column(
crossAxisAlignment: CrossAxisAlignment.start,
...
Icon(
spec.icon,
color: spec.title == 'Files' ? _muted : _neon,
size: 30,
),
const Spacer(),
Text(
...
title: 'Activity',
subtitle: mode == ConnectionMode.connected && api.currentUser != null
? 'Live logs when backend data is available'
: 'Mock activity shown in Local Demo Mode',
...
const SizedBox(height: 16),
const Text(
'Real monitoring/control requires the EMP Control API and EMPControlAgent plugin. The APK opens offline with mock servers so you can preview the dashboard immediately.',
...
decoration: BoxDecoration(
color: _panel.withOpacity(.94),
borderRadius: BorderRadius.circular(26),
border: Border.all(color: Colors.white10),
),
child: Row(
children: DashboardTab.values.map((item) {
        ...
        return Expanded(
          child: InkWell(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                color: selected ? _purple.withOpacity(.35) : Colors.transparent,
                borderRadius: BorderRadius.circular(20),
              ),
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Icon(_tabIcon(item), color: selected ? _neon : _muted),
                  const SizedBox(height: 3),
                  Text(
                    _tabLabel(item),
                    style: TextStyle(
                      color: selected ? _softWhite : _muted,
                      fontSize: 11,
...
static IconData _tabIcon(DashboardTab tab) => switch (tab) {
DashboardTab.dashboard => Icons.dashboard_rounded,
DashboardTab.activity => Icons.receipt_long_rounded,
DashboardTab.security => Icons.shield_rounded,
DashboardTab.about => Icons.info_rounded,
};

static String _tabLabel(DashboardTab tab) => switch (tab) {
...
border: Border.all(
color: subtle ? Colors.white10 : _purple.withOpacity(.36),
),
boxShadow: subtle
? null
: [
...
decoration: BoxDecoration(
color: color.withOpacity(subtle ? .10 : .16),
...
const SizedBox(height: 8),
Text(
'${server.name} • ${demo ? 'Local Demo Mode' : 'Backend Connected'}',
style: const TextStyle(color: _muted),
),
const SizedBox(height: 16),
const Text(
